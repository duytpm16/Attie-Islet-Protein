...
colnames(samples)[grep('Batch',colnames(samples))] <- 'batch'
...
islet_protein_raw <- islet_protein_raw[keep,]
rownames(data.log) <- islet_protein_raw$Mouse.ID
rownames(islet_protein_raw) <- islet_protein_raw$Mouse.ID
islet_protein_raw <- islet_protein_raw[,!(colnames(islet_protein_raw )%in% "Mouse.ID")]


### Removing duplicates in the samples dataframe
...
saveRDS(islet_protein_raw, "attie_islet_protein_filtered_raw.rds")
saveRDS(data.log, "attie_islet_protein_normalized.rds")
saveRDS(data.rz, "attie_islet_protein_rZ_normalized.rds")
...
rm(chr_m_y,ctrl,data.cb,mod,pc.data,batch,chg,dupl,i,rankZ,
   iter,keep,miss,prop.missing,sample,unique.samples, data.compl, wh)
