...
expr      <- dataset.islet.proteins$data$rz
covar     <- dataset.islet.proteins$covar.matrix
med_expr  <- dataset.islet.proteins$data$rz
med_annot <- dataset.islet.proteins$annot.protein %>% dplyr::rename(pos = start)
...
for(i in 1:nrow(lod_df)){
    ...
    print(ggplot(med_melt, aes(x = variable, y = target)) + geom_tile(aes(fill = med_melt$`Proportion Drop`), color = 'black')  + 
            ...
            scale_fill_gradientn(colors = c('cadetblue3','orange','yellow')) + ggtitle(paste0('Chr',hs_chr,' HotSpot Interval Mediation')))
