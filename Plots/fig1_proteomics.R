...
lod.peaks <- dataset.full.islet.proteins$lod.peaks$additive
...
all.chr = lod.peaks %>% select(qtl_chr, gene_chr) %>%
  gather(k, v) %>%
  select(v) %>%
  distinct() %>%
  arrange(v)
...
c <- ggplot(lod_df_cis, aes(x = pos, y = count)) +
                  geom_line(col = "#4286f4") +
                  geom_hline(yintercept = 40, linetype = 2, color = 'grey70') +
                  ylab('No. pQTL/4 Mbp') +
                  xlab('Chromosome') +
...
grid.text(label = 'Distal-pQTL', x=unit(.15,'npc'), y = unit(.97,'npc'),gp=gpar(fontsize=20, fontface = 'bold'))
grid.text(label = 'Local-pQTL', x=unit(.15,'npc'), y = unit(.95,'npc'), gp=gpar(fontsize=20, fontface = 'bold',col="#4286f4"))
...
# Chromosome 2
grid.lines(x = unit(c(0.132, .132), "npc"),
           y = unit(c(0.33, .329), "npc"),
           gp = gpar(fill="black"),
           arrow = arrow(length = unit(0.2, "inches"),type="closed"))
...
# Chromosome 16
grid.lines(x = unit(c(0.798, .798), "npc"),
           y = unit(c(0.33, .329), "npc"),
           gp = gpar(fill="black"),
           arrow = arrow(length = unit(0.2, "inches"),type="closed"))
