...
load("~/Desktop/Attie Mass Spectrometry/Islet/Islet 284 /Version 2/attie_islet_284_qtl_viewer_v2.RData")
lod.peaks <- dataset.islet.proteins.284$lod.peaks$additive
lod.peaks <- lod.peaks[!lod.peaks$cis,]
expr      <- dataset.islet.proteins.284$data$rz
covar     <- dataset.islet.proteins.284$covar.matrix
med_expr  <- dataset.islet.mrna.284$data$rz
med_annot <- dataset.islet.mrna.284$annot.mrna %>% dplyr::rename(pos = start)
med_expr <- med_expr[rownames(expr),med_annot$gene.id]
...
for(i in 1:nrow(lod_df)){
  ...
  stopifnot(sub_lodpeaks$gene.id == colnames(sub_expr))
  
  
  med_in_interval <- med_annot %>% subset(chr == lod_df$chr[i] & abs(pos - lod_df$pos[i]) <= interval) %>% dplyr::arrange(pos)
  med_in_interval_expr <- med_expr[,med_in_interval$gene.id]
  stopifnot(colnames(med_in_interval_expr) == med_in_interval$gene.id)
  ...
  for(j in 1:nrow(order)){
    ...
    med$dp[med$dp < 0] <- 0
    med_result[order$protein.id[j], med$gene.id] <- med$dp
    
  }
  stopifnot(rownames(med_result) == order$protein.id)
  stopifnot(colnames(med_result) == med_in_interval$gene.id)
  ...
  print(ggplot(med_melt, aes(x = variable, y = target)) + geom_tile(aes(fill = med_melt$`Proportion Drop`), color = 'black')  + 
          ...
          scale_fill_gradientn(colors = c('firebrick3','orange','yellow')) + ggtitle(paste0('Chr',hs_chr,' HotSpot Interval Mediation')))
