...
raw     <- read.table("~/Desktop/Attie Mass Spectrometry/Islet/Proteins/DO_islet_proteomics_non_normalized.txt", sep = '\t', header = TRUE)
samples <- read.table("~/Desktop/Attie Mass Spectrometry/attie_DO_sample_annot.txt", header = TRUE ,sep = "\t")
chr_m_y <- read.csv("~/Desktop/Attie Mass Spectrometry/attie_sample_info_ChrM_Y.csv") 
annots  <- readRDS("annotated_uniprots_v2.rds")
...
#   Filter proteins that are not annotated in the 1-19 and X chromosomes
#   Filter proteins without annotations
#
#       raw: 373 x 5415
raw <- raw[,!(colnames(raw) %in% c("Mouse.ID","Injection_order","Plate_number","batch"))]
raw <- raw[,colSums(is.na(raw)) < .50 * nrow(raw)]

annots$uniprot_id <- gsub(';','_',annots$uniprot_id)
annots$uniprot_id <- gsub('-','.',annots$uniprot_id)
annots <- annots[annots$uniprot_id %in% colnames(raw),]
annots <- annots[annots$protein_id != '',]
annots <- annots[!grepl(';',annots$gene_id),]
annots <- annots[annots$chr %in% c(1:19,'X'),]
annots$protein_id[duplicated(annots$protein_id)] <- paste0(annots$protein_id[duplicated(annots$protein_id)],'-2')
rownames(annots) <- annots$protein_id
