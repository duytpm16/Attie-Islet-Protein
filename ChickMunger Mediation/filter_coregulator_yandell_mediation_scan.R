...
for(i in 1:n){
  ...
  mediation_df <- temp %>% 
    ...
    subset(mediation.z.lod < threshold)
...
if(!is.null(chunk_number)){
   saveRDS(new_mediation_df, paste0('attie_islet_',file_name[1],'_filter_coregulator_yandell_intermediate_',file_name[2],'_chunk_',chunk_number,'.rds'))
}else{
   saveRDS(new_mediation_df, paste0('attie_islet_',file_name[1],'_filter_coregulator_yandell_intermediate_',file_name[2],'.rds'))
}
