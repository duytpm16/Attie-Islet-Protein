...
stopifnot(annot.id.med[, id] == colnames(expr.med))
...
for(i in 1:n){
    ...
    med <- mediation_scan(target = expr.targ[, qtl.annot[i], drop = FALSE],
                          mediator = expr.med,
                          annotation = annot.id.med,
                          driver = genoprobs[[qtl.chr[i]]][,,qtl.marker[i]],
                          kinship = K[[qtl.chr[i]]],
                          covar = covar.targ,
    ...
    mediator.start[i]  <- paste0(med$start,  collapse = ',')
...
if(!is.null(chunk_number)){
   saveRDS(mediation_results, paste0('attie_islet_',file_name[1],'_yandell_intermediate_',file_name[2],'_chunk_',chunk_number,'.rds'))
}else{
   saveRDS(mediation_results, paste0('attie_islet_',file_name[1],'_yandell_intermediate_',file_name[2],'.rds'))
}
